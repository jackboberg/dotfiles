---
name: review-changes
description: Review branch changes with Copilot-style analysis. Use when asked to review changes on the current branch, analyze a diff, or get feedback on work before committing.
---

# Review Branch Changes

Analyze all changes on the current branch against the default branch. Produces a structured review in the terminal — no remote API calls.
...
If `git merge-base` fails (no common ancestor, detached HEAD, etc.), fall back to uncommitted changes only:
...
Skip these file types entirely: `package.json`, `package-lock.json`, `yarn.lock`, `pnpm-lock.yaml`, `Gemfile.lock`, `go.sum`, `go.work.sum`, `Cargo.lock`, `*.svg`, `*.log`, `bun.lockb`, any generated or vendored files.
...
Group findings by severity: bugs and security first, then correctness, then readability, then general suggestions.
